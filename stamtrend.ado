...
syntax, vars(string) [name(string) set(string) path(string)  HPSmooth(integer 10)  Year(integer 1990) save hptrend HPYear(string) quite  lag(integer 2) VALyear(string) graph]
...
\*/
	...
	foreach i in `vars' {
		...
		di ""
		local `i'_tfp_gdp_ratio = ``i'tfp'/`tratio`i''
		noi di  as result "	───────────────────────────────────────"
		noi di as text "	ΔTFP/ΔGPD :" as result "  ``i'_tfp_gdp_ratio'"
		di ""
	}
}
/*============================================================================*/
