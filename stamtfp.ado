...
syntax, vars(string) [name(string) set(string) path(string)  HPSmooth(integer 10)  Year(integer 1990) save hptrend HPYear(integer 0) quite lag(integer 2) VALyear(string) graph]
...
\*/
		...
		if "`save'"=="save" {
			graph export "`path'\\`c'tfp.png", replace
		}
	}
